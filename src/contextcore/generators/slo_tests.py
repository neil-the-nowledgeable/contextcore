...
---'''
    ...
    def _generate_chaos_tests(self, project_id: str, requirements: Dict[str, Any], 
        ...
        for target in targets:
            ...
            # Generate chaos tests based on criticality
            if criticality == "high":
                # High criticality: Pod kill and network delay
                pod_chaos_yaml = self.pod_chaos_template.replace("{{name}}", base_name)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{project_id}}", project_id)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{namespace}}", namespace)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{app}}", app_name)
                
                chaos_tests.append(GeneratedTest(
                    name=f"{base_name}-pod-kill",
                    test_type=TestType.CHAOS,
                    derived_from="business.criticality=high",
                    content=pod_chaos_yaml,
                    file_extension=".yaml"
                ))
                
                network_chaos_yaml = self.network_chaos_template.replace("{{name}}", base_name)
                network_chaos_yaml = network_chaos_yaml.replace("{{project_id}}", project_id)
                network_chaos_yaml = network_chaos_yaml.replace("{{namespace}}", namespace)
                network_chaos_yaml = network_chaos_yaml.replace("{{app}}", app_name)
                
                chaos_tests.append(GeneratedTest(
                    name=f"{base_name}-network-delay",
                    test_type=TestType.CHAOS,
                    derived_from="business.criticality=high",
                ...
                # Medium criticality: Pod kill only
                pod_chaos_yaml = self.pod_chaos_template.replace("{{name}}", base_name)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{project_id}}", project_id)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{namespace}}", namespace)
                pod_chaos_yaml = pod_chaos_yaml.replace("{{app}}", app_name)
                ...
                chaos_tests.append(GeneratedTest(
                    name=f"{base_name}-pod-kill",
                    test_type=TestType.CHAOS,
                    derived_from="business.criticality=medium",
                    content=pod_chaos_yaml,
                    file_extension=".yaml"
                ))
            # Low criticality: no chaos tests
